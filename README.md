...
- Smoothing: Spline terms are far more likely to overfit than linear terms. As a result, two variations are fitted per model: one unconstrained and one constrained. The constrained variation is imposed by having non-zero lambdas on all spline terms. With the defaul of having degree 3 polynomial (cubic) and a total of 6 basis funcion (2 knots), the each lambda is chosen to be such that would reduce the degrees of freedom, on each variate, by a fraction of 0.75. Solving for the lambdas require a minimization step (scipy.optimize.minimize).

The results of all models are saved in 'model/workspace/result', identified by the respective cross validation index.
